...
IndiffCurveContainer *ObjectManager::getIndiffCurveContainer()
{
    ...
    if (indiffCurveCont_ == NULL)
    {
        ...
        else
        {
            ...
            else if (curveType == "Linear" || curveType == "Convex" || curveType == "Concave")
            {
...
ShortestPath *ObjectManager::getShPath()
{
    if (shPath_ == NULL)
    {
        ...
        {
            std::cout << " params_->getParamWoSpaces(UseP2PShPathWithRandomReturn) = " << params_->getParamWoSpaces("UseP2PShPathWithRandomReturn") << std::endl;
            if (!params_->getParamWoSpaces("FIXED_PROBABILITY").empty())
            {
                FPType probability = getFloatValue("FIXED_PROBABILITY");
                if (probability <= 0 || probability > 1)
                    throw Error("Unexpected value of parameter <FIXED_PROBABILITY>");
...
LineSearch *ObjectManager::getLineSearch()
{
    if (lineSearch_ == NULL)
    {
        ...
        if (tmp == "BISEC")
        {
            ...
            if (pr <= 0.0)
                throw Error("Line search precision must be positive");
        ...
        else if (tmp == "ARMIJO")
        {
            ...
            if (dec <= 0.0)
                throw Error("Decrement for Armijo search must be positive");
                                          ...
                                          const std::string &dirFlows)
{
    if (algo_ == NULL)
    {
        ...
        else if (getPathAlgoType() != Nothing)
        {
            ...
            if (currPathsAdder_ != NULL)
                currPathsAdder_->setPathSet(getPathSet());
            if (aStar_ != NULL)
                aStar_->setPathSet(getPathSet());
            if (aStarForNonAdd_ != NULL)
                aStarForNonAdd_->setPathSet(getPathSet());
        }
...
NonAddShortestPath *ObjectManager::getNonAddShPath()
{
    ...
    if (nonAddShPath_ == NULL)
    {
        ...
        if (!params_->getParamWoSpaces("UseP2PShPathWithRandomReturn").empty())
        {
            ...
            std::cout << "******************************************: " << (shPath_ == nonAddShPath_) << std::endl;
        }
