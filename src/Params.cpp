...
std::string Params::getAutoFileName(char del)
{
    ...
    std::string attributes[] = {"ALGORITHM", "EQUILIBRATION", "APPROACH", "LINE_SEARCH",
                                ...
                                "ART2_TOLLS", "INDIFF_CURVE_TYPE", "COST_FUNC"};
    ...
    for (int i = 0; i < nbAtt; ++i)
    {
        ...
        if (attributeValue == "yes")
            attributeValue = "withPaths";
        if (attributeValue != "")
            retVal.append(attributeValue + del);
    }
    if (!getParamWoSpaces("USE_PATH_COST_DOMINATION").empty())
        retVal.append("pathCostDomination");
    if (!getParamWoSpaces("UseP2PShPathWithRandomReturn").empty())
        retVal.append("_rndReturn");
    std::string prob = getParamWoSpaces("FIXED_PROBABILITY");
    if (!prob.empty())
        retVal.append("_prob_" + prob);
    return retVal;
};
